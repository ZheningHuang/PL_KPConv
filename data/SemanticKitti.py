...
from data.common import grid_subsampling
...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True):
        ...
        if self.set == 'training':
            self.sequences = ['{:02d}'.format(i) for i in range(11) if i != 8] #02d formats an integer (d) to a field of minimum width 2 (2), with zero-padding on the left (leading 0):
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if exists(batch_lim_file):
            with open(batch_lim_file, 'rb') as file:
                print("batch_limit this is loaded")
