    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        if config.saving:
            ...
            if exists(test_file):
                ...
                with open(test_file, "w") as text_file:
                    text_file.write(line)
            print("val_loader.dataset.use_potentials", val_loader.dataset.use_potentials)
            # Save potentials
            if val_loader.dataset.use_potentials:
                print("saved potentail!")
    ...
    def slam_segmentation_validation(self, net, val_loader, config, debug=True):
        ...
        for i, batch in enumerate(val_loader):
            ...
            for b_i, length in enumerate(lengths):
                ...
                if f_ind % 20 == 0:
                    seq_path = join(val_loader.dataset.path, 'dataset/sequences', val_loader.dataset.sequences[s_ind])
